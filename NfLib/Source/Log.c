...
#define alwayClear _g_log.alwayClear
...
static inline _Log* EndLine(void) {
    ...
    if ((++lineSize) >= LOG_SIZE) { lineSize = LOG_SIZE - 1; }
    for (i = 0; i < lineSize; ++i) { memcpy(&data[lineSize - i], &data[lineSize - i - 1], LOG_LEN); }
    ...
    if ((++lineSize) >= LOG_SIZE) {
        lineSize = LOG_SIZE - 1;
        for (i = 0; i < LOG_SIZE - 1; ++i) { memcpy(&data[i], &data[i + 1], LOG_LEN); }
...
static inline _Log* Add(const char* format, ...) {
    ...
    if (result < 0) { strcpy(log, " <?> "); }
    while (log[i] != '\0') {
        if (lineLen == (LOG_LEN - 1)) { EndLine(); continue; }
        if (log[i] == '\n') { ++i; EndLine(); continue; }
...
static inline void Clear(void) {
    ...
    for (i = 0; i < LOG_SIZE; ++i) { memset(data[i], 0, LOG_LEN); }
    lineSize = 0;
    lineLen = 0;
    clear = false;
}

static inline void HandleClear(void) { if (clear || alwayClear) { clear = false; Clear(); } }
